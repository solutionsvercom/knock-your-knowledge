      ...
      if (!user || user.role !== "admin") {
        ...
        setError(
          "This account is not an administrator. Student and other accounts cannot open the admin panel. " +
            "Sign in here only with an admin account (create one with: npm --prefix server run seed:admin, or set role to admin in the database)."
        );
        return;
      }
        ...
        >
          ...
          {import.meta.env.DEV ? (
            <p className="mt-3 text-xs text-slate-500 rounded-md border border-slate-800 bg-slate-900/80 px-3 py-2">
              ...
              <code className="text-violet-300">npm --prefix server run seed:admin</code>, then sign in as{" "}
