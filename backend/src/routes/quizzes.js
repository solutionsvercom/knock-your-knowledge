...
router.get("/", async (req, res, next) => {
  try {
    ...
    const rows = await Quiz.find({ course_id: String(course_id) }).sort({ createdAt: -1 });
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const row = await Quiz.create(req.body);
...
router.delete("/:id", authMiddleware.required, requireRole("admin", "teacher"), async (req, res, next) => {
  try {
    const row = await Quiz.findByIdAndDelete(req.params.id);
